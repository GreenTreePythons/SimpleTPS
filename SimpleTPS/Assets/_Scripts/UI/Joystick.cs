...
namespace _Scripts.UI
{
    ...
    public class Joystick : MonoBehaviour, 
        ...
        IDragHandler 
    {
        [SerializeField] bool m_DrawJoystickInfoGizmos = false;
        ...
        public void OnPointerDown(PointerEventData eventData)
        {
            ...
            if (m_JoystickType == JoystickType.Static)
            {
                Vector2 localOnBG = GetTouchPosition(
                    m_JoystickBGRectTransform,
                    eventData.position,
                    eventData.pressEventCamera
                );

                float bgRadius = m_JoystickBGRectTransform.rect.width * 0.5f;
                float range = Mathf.Max(bgRadius, m_JoystickHandleMoveRange);

                m_DragStartedInside = (localOnBG.sqrMagnitude <= range * range);
                return;
            }

            Vector2 localOnCanvas = GetTouchPosition(
                m_Canvas.transform as RectTransform,
                eventData.position,
                eventData.pressEventCamera
            );

            m_JoystickBGRectTransform.anchoredPosition = localOnCanvas;
            m_DragStartedInside = true;
        }
        ...
        void OnDrawGizmos()
        {
            if (m_JoystickBGRectTransform != null && m_DrawJoystickInfoGizmos)
            {
